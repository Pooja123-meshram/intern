...
# Regular expression to extract digits from the experience string
def extract_experience(experience):
    match = re.match(r'(\d+)', experience)  # Matches one or more digits at the beginning
    if match:
        return int(match.group(1))  # Returns the integer value of the digits
    return 0  # Default value if no match (e.g., in case of malformed data)
...
def skill_service_candidates(request, service_id):
    ...
    # Filter candidates based on service skills
    matching_candidates = []
    ...
    # Filtering by minimum experience
    if min_experience:
           
        # Ensure the candidate has an 'experience' field and is an integer
        matching_candidates = [
    ...
    if language:
        language_list = [lang.strip().lower() for lang in language.split(',')]
        matching_candidates = [
            candidate for candidate in matching_candidates
            if candidate.languages and any(lang in [l.strip().lower() for l in candidate.languages.split(',')] for lang in language_list)
        ]
    ...
    if city:
        ...
        matching_candidates = [
            candidate for candidate in matching_candidates
            if candidate.city and candidate.city.strip().lower() == city
        ]
    ...
    if education:
        ...
        matching_candidates = [
            candidate for candidate in matching_candidates
            if candidate.education and education in candidate.education.strip().lower()
        ]
